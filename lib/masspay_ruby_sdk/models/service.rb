  ...
  class Service
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MassPayRubySdk::Service` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MassPayRubySdk::Service`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'payers')
        if (value = attributes[:'payers']).is_a?(Array)
          self.payers = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @payers.nil?
        invalid_properties.push('invalid value for "payers", payers cannot be nil.')
      end
    ...
    def hash
      [country_code, delivery_type, payers].hash
    end
